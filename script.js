let currentTabIndex = 0;
    ...
    function renderProfilesList() {
      ...
      profiles.forEach((p, index) => {
        ...
        // ▼▼▼ 追加：ドラッグ＆ドロップのための設定 ▼▼▼
        card.setAttribute('draggable', 'true'); // ドラッグ可能にする
        
        // ドラッグ開始時
        card.addEventListener('dragstart', (e) => {
          draggedProfileItem = p;
          // 見た目を少し半透明にしてドラッグ中であることを表現
          setTimeout(() => card.style.opacity = '0.4', 0);
        });

        // 重なった時（ドロップ先としての装飾）
        card.addEventListener('dragover', (e) => {
          e.preventDefault(); // デフォルトの動作を無効化（ドロップを許可するため必須）
          const rect = card.getBoundingClientRect();
          // マウス/指の位置がカードの半分より上か下かで青い線の位置を変える
        ...
        card.addEventListener('drop', (e) => {
          ...
          if (draggedProfileItem && draggedProfileItem !== p) {
            ...
            // 下半分にドロップされた場合は、ターゲットの1つ後ろに挿入する
            const rect = card.getBoundingClientRect();
            if (e.clientY >= rect.top + rect.height / 2) {
              newIndex++;
            }
        ...
        card.onclick = () => {
          if(!isActive) {
            ...
            appData = profiles.find(x => x.id === currentProfileId).data; currentTabIndex = 0;
            renderApp(); renderProfilesList();
          }
    ...
    const tabObserver = new IntersectionObserver((entries) => { 
      entries.forEach(entry => { 
        if (entry.isIntersecting) { 
          ...
          if (activeTab) { activeTab.classList.add('active'); activeTab.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' }); currentTabIndex = idx; } 
        } 
      }); 
    ...
    async function openJsonSelectModal(targetFolderIndex) {
      ...
      // CORSエラーやfetch失敗時は内蔵のdefaultAppDataを使用
    ...
    function renderApp() {
      ...
      appData.forEach((folder, index) => {
        ...
        addItemBtn.addEventListener('click', () => {
          // openEditModal にコールバック（手動追加用）と、onPresetClick（リストから選ぶ用）を渡す
        ...
        folderDiv.appendChild(folderInner); sliderContainer.appendChild(folderDiv); tabObserver.observe(folderDiv);
      });
      ...
      setTimeout(() => { if(sliderContainer.children.length > currentTabIndex) sliderContainer.scrollLeft = sliderContainer.clientWidth * currentTabIndex; }, 0);
    }
    renderApp();
    ...
    document.getElementById('menuEdit').addEventListener('click', () => {
      ...
      if (type === 'item') {
        // 編集時はプリセットから選ぶボタン(onPresetClick)を渡さないため null を指定
